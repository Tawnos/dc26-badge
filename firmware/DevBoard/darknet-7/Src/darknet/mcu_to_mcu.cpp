...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef* huart) {
  if (huart == DarkNet7::instance->getMcuToMcu().getUART()) {
    DarkNet7::instance->getMcuToMcu().handleMcuToMcu();
  }
}

void HAL_UART_ErrorCallback(UART_HandleTypeDef* huart) {
  if (huart == DarkNet7::instance->getMcuToMcu().getUART()) {
    DarkNet7::instance->getMcuToMcu().onError();
  }
}

void HAL_UART_TxCpltCallback(UART_HandleTypeDef* huart) {
  if (huart == DarkNet7::instance->getMcuToMcu().getUART()) {
    DarkNet7::instance->getMcuToMcu().onTransmitionComplete();
  }
}
...
MCUToMCU& DarkNet7::instance->getMcuToMcu() {
  if (0 == mSelf) {
    mSelf = new MCUToMCU();
  }
  return *mSelf;
